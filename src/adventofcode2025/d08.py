...
def part2(input_: Iterable[str]) -> int:
    ...
    edges = list((dist3(p1, p2), p1, p2) for p1, p2 in combinations(points, 2))
    heapq.heapify(edges)

    adjacent_map = defaultdict(list)

    def get_cluster(p0: Tuple[int, int, int]) -> Iterable[Tuple[int, int, int]]:
        if p0 not in adjacent_map:
            return [p0]
        cluster = {p0}
        queue = [p0]
        while queue:
    ...
    root_cluster = set(heapq.heappop(edges)[1:])
    while edges:
        _, p1, p2 = heapq.heappop(edges)
        if p1 in root_cluster and p2 in root_cluster:
            continue
        elif p1 in root_cluster:
            root_cluster.update(get_cluster(p2))
        elif p2 in root_cluster:
            root_cluster.update(get_cluster(p1))
        else:
            adjacent_map[p1].append(p2)
            adjacent_map[p2].append(p1)
        if len(root_cluster) == len(points):
            return p1[0] * p2[0]
